...
try:
    ...
    with open(argv[1]) as f:
        ...
        cir_def = extract_circuit(lines) #extract ciruit defination from netlist file
        cir_def = remove_comments(cir_def) # removes comment

        nodes_dict = nodes_dictionary(cir_def) # stores nodes in dictionary
        voltage_dict = make_dict(cir_def,'v') # stores nodes in dictionary
        inductor_dict = make_dict(cir_def,'l') # stores nodes in dictionary
        n,k = nodes_voltage_count(cir_def) # number of nodes and voltages required for matrix dimension
        w = get_freq(lines) 
